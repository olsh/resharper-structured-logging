...
Unlike the analyzers above, a context action reports nothing and highlights nothing. It sits in the
<kbd>Alt</kbd>+<kbd>Enter</kbd> menu and waits to be asked.

### Convert to LoggerMessage

Rewrites a `Microsoft.Extensions.Logging` call into a method generated by the `LoggerMessage` source
generator, which is the rewrite
[CA1848](https://learn.microsoft.com/en-us/dotnet/fundamentals/code-analysis/quality-rules/ca1848) asks for
and offers no fix for. Put the caret on the call and pick *Convert to LoggerMessage*:

```csharp
// before
_logger.LogInformation("Order {OrderId} shipped to {Country}", order.Id, address.Country);

// after
Log.OrderShipped(_logger, order.Id, address.Country);

internal static partial class Log
{
    [LoggerMessage(Level = LogLevel.Information, Message = "Order {OrderId} shipped to {Country}")]
    public static partial void OrderShipped(ILogger logger, int orderId, string country);
}
```

The method name is derived from the words of the template and left as a rename hotspot, so a better one is
a keystroke away. Each template hole becomes a parameter named after it and typed after the argument that
filled it, the exception argument becomes an `Exception` parameter, and a level the call computes rather
than states becomes a `LogLevel` parameter; the generator binds those two by type. A constant `EventId` is
carried over, and a call that passes none generates none, which the generator has allowed since .NET 8.

The method joins a `static partial class` in the namespace whose members already carry `[LoggerMessage]`,
and only when there is none is `Log` created next to the type doing the logging.
